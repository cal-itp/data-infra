...
# source zip and differing only in configuration. Nothing here touches the standard
# lane in service.tf / workflow.tf: the cohort feeds keep running on the standard
# 20s clock as well, so the RT dataset never develops a gap for a study agency.
#
...
resource "google_storage_bucket" "gtfs-rt-raw-high-frequency" {
  name     = "calitp-staging-gtfs-rt-raw-high-frequency"
  ...
  # Deliberately no retention_policy: study data has to stay deletable, and a
  # retention period would leave a misconfiguration's objects stranded until it
  # expired. Note the consequence -- with nothing blocking replacement, a
  # colliding object path overwrites silently, which is why the archiver function
  # below runs with CALITP_GTFS_RT_FAIL_ON_OVERWRITE.
  lifecycle_rule {
    action {
      type = "Delete"
    }
...
resource "google_pubsub_topic" "gtfs-rt-archiver-staging-high-frequency-clock" {
  count = local.high_frequency_enabled

  name    = "gtfs-rt-archiver-staging-high-frequency-clock"
  project = "cal-itp-data-infra-staging"
}

resource "google_pubsub_topic" "gtfs-rt-archiver-staging-high-frequency-heartbeat" {
  count = local.high_frequency_enabled

  name    = "gtfs-rt-archiver-staging-high-frequency-heartbeat"
  project = "cal-itp-data-infra-staging"
}

resource "google_pubsub_topic" "gtfs-rt-archiver-staging-high-frequency" {
  count = local.high_frequency_enabled

  name    = "gtfs-rt-archiver-staging-high-frequency"
  project = "cal-itp-data-infra-staging"
}

resource "google_eventarc_trigger" "gtfs-rt-archiver-staging-high-frequency-clock" {
  count = local.high_frequency_enabled
  ...
  destination {
    workflow = google_workflows_workflow.gtfs-rt-archiver-staging-high-frequency-clock[0].id
  }

  transport {
    pubsub {
      topic = google_pubsub_topic.gtfs-rt-archiver-staging-high-frequency-clock[0].id
    }
  }
}

resource "google_workflows_workflow" "gtfs-rt-archiver-staging-high-frequency-clock" {
  count = local.high_frequency_enabled
  ...
  # Quieter than the standard clock's LOG_ALL_CALLS / EXECUTION_HISTORY_DETAILED:
  ...
  user_env_vars = {
    "CALITP_TOPIC__GTFS_RT_ARCHIVER_HEARTBEAT" = google_pubsub_topic.gtfs-rt-archiver-staging-high-frequency-heartbeat[0].id
  }
}

resource "google_cloud_scheduler_job" "gtfs-rt-archiver-staging-high-frequency-clock" {
  count = local.high_frequency_enabled
  ...
  # Every minute, unlike the standard staging clock's hourly "4 * * * *"
  # (workflow.tf). Staging is throttled to keep it cheap, but a soak of this lane
  # has to actually exercise the per-minute tick loop to prove the cadence and the
  # object-path fix, so it runs on the same schedule as production would.
  schedule = "* * * * *"

  pubsub_target {
    topic_name = google_pubsub_topic.gtfs-rt-archiver-staging-high-frequency-clock[0].id
...
resource "google_cloudfunctions2_function" "gtfs-rt-archiver-staging-high-frequency-heartbeat" {
  count = local.high_frequency_enabled
  ...
  service_config {
    ...
    environment_variables = {
      ...
      CALITP_TOPIC__GTFS_RT_ARCHIVER      = google_pubsub_topic.gtfs-rt-archiver-staging-high-frequency[0].id
  ...
  event_trigger {
    ...
    pubsub_topic   = google_pubsub_topic.gtfs-rt-archiver-staging-high-frequency-heartbeat[0].id
...
resource "google_cloudfunctions2_function" "gtfs-rt-archiver-staging-high-frequency" {
  count = local.high_frequency_enabled
  ...
  service_config {
    ...
    # misconfigured cohort throttles only the study, never the standard archiver's
    # 300-instance pool, and bounds the lane's compute spend structurally rather
    # than by policy.
    ...
    environment_variables = {
      ...
      # Must stay below the cadence. At the standard lane's 10s, up to four
      ...
      # Deliberately not set on the standard lane, where benign Pub/Sub
  ...
  event_trigger {
    trigger_region        = "us-west2"
    event_type            = "google.cloud.pubsub.topic.v1.messagePublished"
    pubsub_topic          = google_pubsub_topic.gtfs-rt-archiver-staging-high-frequency[0].id
    retry_policy          = "RETRY_POLICY_DO_NOT_RETRY"
    service_account_email = data.terraform_remote_state.iam.outputs.google_service_account_gtfs-rt-archiver_email
  }
}
