output "google_compute_firewall_tfer--default-allow-http_self_link" {
  value = "${google_compute_firewall.tfer--default-allow-http.self_link}"
}

output "google_compute_firewall_tfer--default-allow-https_self_link" {
  value = "${google_compute_firewall.tfer--default-allow-https.self_link}"
}

output "google_compute_firewall_tfer--default-allow-icmp_self_link" {
  value = "${google_compute_firewall.tfer--default-allow-icmp.self_link}"
}

output "google_compute_firewall_tfer--default-allow-internal_self_link" {
  value = "${google_compute_firewall.tfer--default-allow-internal.self_link}"
}

output "google_compute_firewall_tfer--default-allow-rdp_self_link" {
  value = "${google_compute_firewall.tfer--default-allow-rdp.self_link}"
}

output "google_compute_firewall_tfer--default-allow-ssh_self_link" {
  value = "${google_compute_firewall.tfer--default-allow-ssh.self_link}"
}

output "google_compute_firewall_tfer--gke-data-infra-apps-0fe1e974-all_self_link" {
  value = "${google_compute_firewall.tfer--gke-data-infra-apps-0fe1e974-all.self_link}"
}

output "google_compute_firewall_tfer--gke-data-infra-apps-0fe1e974-ssh_self_link" {
  value = "${google_compute_firewall.tfer--gke-data-infra-apps-0fe1e974-ssh.self_link}"
}

output "google_compute_firewall_tfer--gke-data-infra-apps-0fe1e974-vms_self_link" {
  value = "${google_compute_firewall.tfer--gke-data-infra-apps-0fe1e974-vms.self_link}"
}

output "google_compute_firewall_tfer--gke-us-west2-calitp-airflow2-pr-171e4e47-gke-0b2083d6-ssh_self_link" {
  value = "${google_compute_firewall.tfer--gke-us-west2-calitp-airflow2-pr-171e4e47-gke-0b2083d6-ssh.self_link}"
}

output "google_compute_firewall_tfer--gke-us-west2-calitp-airflow2-pr-88ca8ec6-gke-98ed8a64-all_self_link" {
  value = "${google_compute_firewall.tfer--gke-us-west2-calitp-airflow2-pr-88ca8ec6-gke-98ed8a64-all.self_link}"
}

output "google_compute_firewall_tfer--gke-us-west2-calitp-airflow2-pr-88ca8ec6-gke-98ed8a64-exkubelet_self_link" {
  value = "${google_compute_firewall.tfer--gke-us-west2-calitp-airflow2-pr-88ca8ec6-gke-98ed8a64-exkubelet.self_link}"
}

output "google_compute_firewall_tfer--gke-us-west2-calitp-airflow2-pr-88ca8ec6-gke-98ed8a64-inkubelet_self_link" {
  value = "${google_compute_firewall.tfer--gke-us-west2-calitp-airflow2-pr-88ca8ec6-gke-98ed8a64-inkubelet.self_link}"
}

output "google_compute_firewall_tfer--gke-us-west2-calitp-airflow2-pr-88ca8ec6-gke-98ed8a64-vms_self_link" {
  value = "${google_compute_firewall.tfer--gke-us-west2-calitp-airflow2-pr-88ca8ec6-gke-98ed8a64-vms.self_link}"
}

output "google_compute_firewall_tfer--k8s-ad40eb3afc69e4deeaf43a1ed1393eeb-http-hc_self_link" {
  value = "${google_compute_firewall.tfer--k8s-ad40eb3afc69e4deeaf43a1ed1393eeb-http-hc.self_link}"
}

output "google_compute_firewall_tfer--k8s-e092126e72b02003-node-http-hc_self_link" {
  value = "${google_compute_firewall.tfer--k8s-e092126e72b02003-node-http-hc.self_link}"
}

output "google_compute_firewall_tfer--k8s-fw-a0d3bb39e959b49a1b6015fd2193e30d_self_link" {
  value = "${google_compute_firewall.tfer--k8s-fw-a0d3bb39e959b49a1b6015fd2193e30d.self_link}"
}

output "google_compute_firewall_tfer--k8s-fw-ad40eb3afc69e4deeaf43a1ed1393eeb_self_link" {
  value = "${google_compute_firewall.tfer--k8s-fw-ad40eb3afc69e4deeaf43a1ed1393eeb.self_link}"
}
