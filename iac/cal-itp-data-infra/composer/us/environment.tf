  ...
  config {
    ...
    software_config {
      ...
      env_variables = merge(local.env_variables, {
        ...
        "CALITP_BUCKET__TIDES"                                 = "gs://${data.terraform_remote_state.gcs.outputs.google_storage_bucket_calitp-tides_name}",
...
resource "google_composer_environment" "calitp-composer3" {
