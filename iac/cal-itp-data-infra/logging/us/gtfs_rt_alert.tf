...
resource "google_monitoring_alert_policy" "gtfs_rt_bucket_writes_alert" {
  ...
  conditions {
    ...
    condition_sql {
      query = <<-QUERY
      SELECT count(*) AS row_count FROM `projects/${data.google_project.project.project_id}/logs/cloudaudit.googleapis.com%2Fdata_access`
  ...
  documentation {
    content   = "This alert triggers when the number of writes to 'calitp-gtfs-rt-raw-v2/trip_updates' is less than 7500 in 15 minutes."
