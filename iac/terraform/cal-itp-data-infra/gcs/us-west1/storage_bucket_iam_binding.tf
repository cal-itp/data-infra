...
resource "google_storage_bucket_iam_binding" "tfer--calitp-gtfs-schedule-validation" {
  ...
  members = ["serviceAccount:gtfs-rt-archiver@cal-itp-data-infra.iam.gserviceaccount.com"]
...
resource "google_storage_bucket_iam_binding" "tfer--test-calitp-gtfs-download-config" {
  ...
  members = ["serviceAccount:dev-gtfs-rt-archiver-v3@cal-itp-data-infra.iam.gserviceaccount.com"]
  role    = "roles/storage.objectViewer"
}
