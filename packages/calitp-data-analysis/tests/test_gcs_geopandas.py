...
def test_read_parquet(mocker, gcs_auth_setup, gcs_geopandas):
    ...
    geopandas.read_parquet.assert_called_once_with(PATH, ANY_OTHER_ARGUMENT, storage_options={'token': TOKEN})
...
def test_geo_data_frame_to_parquet(mocker, gcs_auth_setup, gcs_geopandas):
    ...
    gcsfs.GCSFileSystem.assert_called_once_with(token=TOKEN)
    geo_data_frame.to_parquet.assert_called_once_with(PATH, ANY_OTHER_ARGUMENT, filesystem=GCS_FILESYSTEM)