...
After that, you should create a sync task (configured via a small YAML file) in [the sync_littlepay_v3 DAG folder in the data-infra repository](https://github.com/cal-itp/data-infra/tree/main/airflow/dags/sync_littlepay_v3), structurally similar to the others that are there, utilizing the instance name and access key name for the agency you're setting up. Similarly, you should create a parse task (configured via another YAML file) corresponding to the new agency in [the parse_littlepay_v3 DAG folder](https://github.com/cal-itp/data-infra/tree/main/airflow/dags/parse_littlepay_v3). Then, add a mapping between the agency's instance identifier and its Cal-ITP internal GTFS dataset identifier in [this seed file](https://github.com/cal-itp/data-infra/blob/main/warehouse/seeds/payments_entity_mapping.csv).
