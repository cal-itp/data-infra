...
Parsed data outcomes path (wrt bucket) | N/A | One path per file (ex. `agency.txt_ parsing_results/`) | One path per RT feed type (ex. `vehicle_ positions_ outcomes/`) | [`parse_ littlepay_ job_result/`]('https://console.cloud.google.com/storage/browser/calitp-payments-littlepay-parsed/parse_littlepay_job_result?project=cal-itp-data-infra&supportedpurview=project&pageState=(%22StorageObjectListTable%22:(%22f%22:%22%255B%255D%22))&prefix=&forceOnObjectsSortingFiltering=false') | N/A
