...
JAR_DEFAULT = os.path.normpath(
    os.path.join(
        os.path.dirname(os.path.realpath(__file__)), os.environ["GTFS_RT_VALIDATOR_JAR"]
    )
)
...
class RTHourlyAggregation(PartitionedGCSArtifact):
    ...
    def table(self):
        if self.step == RTProcessingStep.parse:
            return self.feed_type.value
        if self.step == RTProcessingStep.validate:
            return f"{self.feed_type.value}_validation_notices"
        raise RuntimeError("we should not be here")
...
class GTFSRTJobResult(PartitionedGCSArtifact):
    ...
    def table(self):
        if self.step == RTProcessingStep.parse:
            return f"{self.feed_type.value}_outcomes"
        if self.step == RTProcessingStep.validate:
            return f"{self.feed_type.value}_validation_outcomes"
        raise RuntimeError("we should not be here")
...
def main(
    ...
    typer.secho(
        f"found {len(hourly_feed_files.files)} {feed_type.value} files in {len(aggregated_feed.get_aggregates())} aggregations to process",
        fg=typer.colors.MAGENTA,
    )

    if base64url:
        typer.secho(
            f"url filter applied, only processing {base64url} ({base64.urlsafe_b64decode(base64url)})", fg=typer.colors.YELLOW
        )
