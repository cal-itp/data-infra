...
class Heartbeat:
    def future_callback(
        self, logger: logging.Logger, publish_future: pubsub_v1.publisher.futures.Future
    ) -> None:
    ...
    def path(self) -> str:
        return os.path.join("projects", self.project_id(), "topics", self.topic_name())

    def run(
        self, publisher=pubsub_v1.PublisherClient()
    ) -> list[pubsub_v1.publisher.futures.Future]:
        return [
            publisher.publish(self.path(), data=message) for message in self.messages()
        ]
