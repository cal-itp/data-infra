...
The publication script does _not_ read from that YAML file directly when publishing - it reads from the manifest file generated by the [deploy-dbt workflow](https://github.com/cal-itp/data-infra/blob/main/.github/workflows/deploy-dbt.yml) whenever someone makes changes to dbt models. By default, that manifest file is read from the GCS bucket.
...
converted into appropriate CSVs and written to [GCS bucket calitp-publish](https://console.cloud.google/storage/browser/calitp-publish). By default, the script will read the latest `manifest.json`.

When dbt model changes are merged to the main branch, a new version of `manifest.json` is automatically generated for tables in the production warehouse by the [deploy-dbt workflow](https://github.com/cal-itp/data-infra/blob/main/.github/workflows/deploy-dbt.yml) in [Github Actions](https://github.com/cal-itp/data-infra/actions), and placed inside the dbt-docs GCS bucket.
