...
if [[ $prev_tag ]]; then
  ...
  test ! -e "$proj_root/services/$BUILD_APP" || changelog_paths+=("$proj_root/services/$BUILD_APP")
  test ! -e "$proj_root/kubernetes/apps/manifests/$BUILD_APP" || changelog_paths+=("$proj_root/kubernetes/apps/manifests/$BUILD_APP")
  test ! -e "$proj_root/kubernetes/apps/charts/$BUILD_APP" || changelog_paths+=("$proj_root/kubernetes/apps/charts/$BUILD_APP")
  test ! -e "$proj_root/kubernetes/apps/values/$BUILD_APP.yaml" || changelog_paths+=("$proj_root/kubernetes/apps/values/$BUILD_APP.yaml")

  while read overlay; do
    changelog_paths+=("$overlay")
  done <<< $(find $proj_root/kubernetes/apps/overlays -maxdepth 1 -type d -name "$BUILD_APP-*")

  tag_msg=$(git shortlog "$prev_tag.." -- "${changelog_paths[@]}")
