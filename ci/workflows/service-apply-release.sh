...
test "$REPO_TAG"                || REPO_TAG=$(git describe --abbrev=0)
test "$BUILD_APP"               || BUILD_APP=$(git tag -l --format='%(contents)' "$REPO_TAG" | grep '^BUILD_APP=' | cut -d= -f2-)
test "$BUILD_ID"                || BUILD_ID=$(git tag -l --format='%(contents)' "$REPO_TAG" | grep '^BUILD_ID=' | cut -d= -f2-)
test "$BUILD_REPO"              || BUILD_REPO=$(git tag -l --format='%(contents)' "$REPO_TAG" | grep '^BUILD_REPO=' | cut -d= -f2-)
