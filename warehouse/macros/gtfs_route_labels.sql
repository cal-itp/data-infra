{% macro parse_route_id(operator_name_column, route_id_column) %}
    CASE
        ...
        WHEN CONTAINS_SUBSTR( {{ operator_name_column  }}, "Roseville")
            THEN TRIM(SPLIT( {{ route_id_column }}, "_")[SAFE_OFFSET(0)])
...
{% macro get_combined_route_name(operator_name_column, route_id_column, route_short_name_column, route_long_name_column) %}
...
-- https://github.com/cal-itp/data-analyses/blob/59dff810bc963de265709e803a166bc83d5aa3d7/rt_segment_speeds/segment_speed_utils/time_series_utils.py#L42-L105
    CASE
        ...
        WHEN (
            ...
            CONCAT(
                {{ parse_route_id(operator_name_column, route_id_column) }},
                "__",
                COALESCE( {{ route_long_name_column }}, "")
            )
        )
        ELSE TRIM(
            CONCAT(
                {{ parse_route_id(operator_name_column, route_id_column) }},
                "__",
                COALESCE( {{ route_short_name_column }}, ""),
                " ",
                COALESCE( {{ route_long_name_column }}, "")
            )
        )
