...
int_gtfs_schedule__deduped_fare_rules AS (
    SELECT *
    FROM {{ ref('int_gtfs_schedule__deduped_fare_rules') }}
),

make_dim AS (
{{ make_schedule_file_dimension_from_dim_schedule_feeds('dim_schedule_feeds', 'int_gtfs_schedule__deduped_fare_rules') }}
),
