...
WITH source AS (
    SELECT * FROM {{ ref("stg_ntd__service_by_mode_and_time_period") }}
)

SELECT {{ dbt_utils.generate_surrogate_key(['report_year', 'ntd_id', 'type_of_service', 'mode', 'time_period']) }} as key,
       report_year,
       type_of_service,
       ntd_id,
       agency,
