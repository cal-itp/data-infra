...
| No expired services are listed in the feed | Best Practices Alignment | calendars.txt does not contain any rows with an end_date that is in the past, and calendar_dates.txt does not contain any service_id's for which all dates are in the past. |
{% enddocs %}
