WITH
-- selecting the distinct trips from GTFS Vehicle Postions for 3 orgs over 2 months
vp_trips AS (
    SELECT DISTINCT
        calitp_itp_id,
        calitp_url_number,
...
--- selecting GTFS schedule data for all 3 orgs over 2 months
sched_trips AS (
    SELECT
        trip_id,
        route_id,
    ...
    WHERE service_date BETWEEN '2022-08-01' AND '2022-09-30'
        AND (calitp_itp_id IN (4, 278, 282) AND is_in_service = True)
),
...
gtfs_rt_vs_schedule_trips_transitcenter_sample AS (
    SELECT
        T1.calitp_itp_id,
        T2.agency_name,
        T1.calitp_url_number,
...
SELECT *
FROM gtfs_rt_vs_schedule_trips_transitcenter_sample
