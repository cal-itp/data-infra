...
gtfs_schedule_data_feed_trip_stops_latest AS (
    ...
    FROM gtfs_schedule_index_feed_trip_stops AS Indx
    LEFT JOIN gtfs_schedule_dim_feeds AS Feed USING (feed_key)
    LEFT JOIN gtfs_schedule_dim_routes AS Route USING (route_key)
    LEFT JOIN gtfs_schedule_dim_trips AS Trip USING (trip_key)
    LEFT JOIN gtfs_schedule_dim_stop_times USING (stop_time_key)
    LEFT JOIN gtfs_schedule_dim_stops AS Stop USING (stop_key)
    WHERE
