...
gtfs_schedule_stg_calendar_long AS (
    SELECT *
    FROM {{ ref('gtfs_schedule_stg_calendar_long') }}
),
...
cal_dates AS (
    ...
    FROM (
        ...
        SELECT DISTINCT * FROM calendar_dates_clean
    )
),
...
calendar_daily AS (
    ...
    FROM gtfs_schedule_stg_calendar_long AS t1
    INNER JOIN dim_date AS t2
        ON
...
gtfs_schedule_stg_daily_service AS (
    ...
    FROM calendar_daily AS t1
    FULL JOIN date_include AS t2 USING (calitp_itp_id, calitp_url_number, service_id, service_date)
    FULL JOIN date_exclude AS t3 USING (calitp_itp_id, calitp_url_number, service_id, service_date)
    -- TODO: remove hardcoding--set this to be 1 month in the future, etc..
    WHERE service_date < DATE_ADD(CURRENT_DATE(), INTERVAL 1 YEAR)
)

SELECT * FROM gtfs_schedule_stg_daily_service
