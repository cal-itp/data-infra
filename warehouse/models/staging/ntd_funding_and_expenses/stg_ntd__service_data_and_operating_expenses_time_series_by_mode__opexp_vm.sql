WITH source AS (
    SELECT *
    FROM {{ source('external_ntd__funding_and_expenses', 'historical__service_data_and_operating_expenses_time_series_by_mode__opexp_vm') }}
),
...
stg_ntd__service_data_and_operating_expenses_time_series_by_mode__opexp_vm AS (
    SELECT
...
SELECT * FROM stg_ntd__service_data_and_operating_expenses_time_series_by_mode__opexp_vm
