WITH raw_validations_outcomes AS (
    ...
    FROM {{ source('external_gtfs_rt', 'trip_updates_validations_outcomes') }}
),

stg_gtfs_rt__trip_updates_validations_outcomes AS (
    SELECT
...
SELECT * FROM stg_gtfs_rt__trip_updates_validations_outcomes
