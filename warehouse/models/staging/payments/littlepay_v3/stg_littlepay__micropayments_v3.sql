...
clean_columns AS (
    SELECT
        ...
        -- this field is deprecated in v3, previously weren't being populated
        -- but adding it here as a null string for the union with v1, so as to not risk breaking dashboards
        TIMESTAMP({{ trim_make_empty_string_null("''") }}) AS transaction_time,
        ...
        {{ dbt_utils.generate_surrogate_key(['participant_id',
            'aggregation_id', 'micropayment_id', 'customer_id', 'funding_source_id',
...
add_surrogate_keys AS (
    ...
    -- NEED TO REPLACE transaction_time IN DEDUPE BELOW AND UNCOMMENT BECAUSE IT IS NOT IN NEW SCHEMA!!!
