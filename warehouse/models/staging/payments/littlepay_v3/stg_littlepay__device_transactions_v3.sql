WITH source AS (
    SELECT * FROM {{ source('external_littlepay_v3', 'device_transactions') }}
),
...
clean_columns AS (
    SELECT
        ...
        -- these fields are deprecated in v3, previously weren't being populated
        -- but adding it here as a null string for the union with v1, so as to not risk breaking dashboards
        {{ trim_make_empty_string_null("''") }} AS location_scheme,
        {{ trim_make_empty_string_null("''") }} AS transaction_deny_reason,
        ...
        -- removed 'transaction_deny_reason' and 'location_scheme' from hash because they are not included in feed v3
        {{ dbt_utils.generate_surrogate_key(['participant_id', 'customer_id',
            ...
            'type', 'transaction_outcome', 'transaction_timestamp_utc',
            'location_id', 'location_name', 'zone_id', 'route_id', 'mode',
...
stg_littlepay__device_transactions_v3 AS (
    SELECT
        participant_id,
        customer_id,
...
SELECT * FROM stg_littlepay__device_transactions_v3
