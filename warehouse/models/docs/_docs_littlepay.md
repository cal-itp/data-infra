...
{% docs lp_feed_version %}
The Littlepay data feed version (e.g. 'v1') that the data came from.
{% enddocs %}

{% docs lp_instance %}
The Littlepay customer instance that the data came from.
Often the same as `participant_id` except some agencies are in the `cal-itp` instance.
{% enddocs %}

{% docs lp_extract_filename %}
The file name from which this data was sourced (the name of the file that was ingested from Littlepay.)
{% enddocs %}
