...
shapes_clean AS (
    ...
    SELECT
        ...
        SAFE_CAST(TRIM(shape_pt_lat) AS NUMERIC) AS shape_pt_lat,
        SAFE_CAST(TRIM(shape_pt_lon) AS NUMERIC) AS shape_pt_lon,
        SAFE_CAST(TRIM(shape_pt_sequence) AS INT64) AS shape_pt_sequence,
        SAFE_CAST(TRIM(shape_dist_traveled) AS NUMERIC) AS shape_dist_traveled,
        calitp_extracted_at,
        calitp_hash,
