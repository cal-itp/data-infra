...
gtfs_schedule_dim_routes AS (
    SELECT * FROM {{ ref('gtfs_schedule_dim_routes') }}
),
...
stg_cleaned_micropayment_adjustments AS (
    SELECT * FROM {{ ref('stg_cleaned_micropayment_adjustments') }}
),
...
stg_cleaned_device_transaction_types AS (
    SELECT * FROM {{ ref('stg_cleaned_device_transaction_types') }}
),
...
gtfs_routes_with_participant AS (
    ...
    INNER JOIN payments_feeds USING (calitp_itp_id, calitp_url_number)
),
