...
dim_calendar AS (
    SELECT  *,
            ...
            -- An example value of this field is "0000011"
...
stop_times_expanded AS (
    SELECT t1.feed_key,
           t1.trip_id,
           t1.stop_sequence,
           CONCAT(t1.arrival_time,t1.departure_time,t2.stop_lat,t2.stop_lon) AS stop_info
...
stop_times_agg AS (
    SELECT feed_key,
           ...
           STRING_AGG(stop_info ORDER BY stop_sequence ASC) AS stop_info_agg
...
trips_expanded AS (
    SELECT t1.feed_key,
           ...
           CONCAT(t2.stop_info_agg,t3.day_combo) AS trip_info,
           t3.start_date,
           t3.end_date
      FROM dim_trips t1
...
trips_version_history AS (
  SELECT t1.base64_url,
   ...
   WHERE t1.prev_feed_key IS NOT null
),
...
trips_version_compare AS (
  SELECT
         ...
         trips.trip_info,
         prev_trips.trip_info AS prev_trip_info,
...
daily_improper_trips_updates AS (
  SELECT base64_url,
         ...
         COUNT(CASE WHEN prev_trip_info IS null THEN 1 END) AS trip_added,
          -- An existing trip is being removed
         COUNT(CASE WHEN trip_info IS null THEN 1 END) AS trip_removed,
          -- An existing trip's is being changed
         COUNT(CASE WHEN trip_info != prev_trip_info THEN 1 END) AS trip_changed
...
calendar_dates_version_history AS (
  SELECT t1.base64_url,
         t1.feed_key,
         t1.prev_feed_key,
         t1.next_feed_key,
...
calendar_dates_version_compare AS (
  SELECT
    ...
    FROM calendar_dates_version_history AS cal_dates
    FULL OUTER JOIN calendar_dates_version_history AS prev_cal_dates
      ON cal_dates.prev_feed_key = prev_cal_dates.feed_key
...
daily_improper_calendar_dates_updates AS (
  SELECT base64_url,
    ...
    FROM calendar_dates_version_compare
   WHERE
